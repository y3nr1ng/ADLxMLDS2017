...
from keras.layers import Conv2D, Convolution2D, Flatten, Dense
...
NUM_EPISODES = 100000 #12000  # Number of episodes the agent plays
...
class Agent_DQN(Agent):
    ...
    def init_game_setting(self):
        ...
        self.first_run = True
        self.initial_state = self.get_initial_state(observation, self.last_observation)
    ...
    def make_action(self, observation, test=True):
        ...
        if self.epsilon >= random.random() or self.t < INITIAL_REPLAY_SIZE:
            ...
            action = np.argmax(self.q_values.eval(feed_dict={self.s: [np.float32(observation / 255.0)]}))
